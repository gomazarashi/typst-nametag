...
今回は1つのPDFファイルに全員分の名札をまとめて出力したのち、必要があればPythonスクリプトを用いて参加者ごとにPDFファイルを分割する、という流れを採用します。
...
今回のテンプレートでは、以下のフォントを使用しています。

- **Roboto**: 英数字用
- **IBM Plex Sans JP**: 日本語用
- **Roboto Mono**: SNSアカウント名用

これらのフォントは[Google Fonts](https://fonts.google.com/)から無料でダウンロード可能です。
...
```text
nametags
├── nametag_template.typ    # テンプレート
├── all_nametags.typ        # 単一PDF出力用
├── participants.csv      # 参加者情報
├── split_pdf.py          # PDF分割用スクリプト
├── assets                # 画像等のアセット
│   └── logo.png          # イベントロゴ画像
└── output                # 出力先
    ├── all_nametags.pdf    # 単一PDFファイル
    └── single_nametags     # 上記のPDFを分割したもの
```

各ディレクトリ/ファイルの役割は以下の通りです。

- nametag_template.typ
名札1枚分（A6縦）のレイアウトとロジックを定義するTypstファイルです。氏名・所属・役割ラベル・イベント名・ロゴの配置やスタイルをまとめておき、`all_nametags.typ`からインポートして利用します。

- all_nametags.typ
participants.csvから全参加者分のデータをcsv()で読み込み、1人につき1ページの名札を生成して、複数ページからなる1つのPDF(output/all_nametags.pdf)を出力するためのTypstファイルです。

- participants.csv
参加者一覧を保持するCSVファイルです。

- split_pdf.py
参加者ごとにPDFファイルを分割して出力するPythonスクリプトです。

- assets/logo.png
名札に表示するイベントロゴ画像です。nametag_template.typからimage("assets/logo.png",...)のように読み込みます。

- output/
生成されたPDFファイルの出力先ディレクトリです。all_nametags.pdfと、`split_pdf.py`で生成される参加者ごとのPDFファイル(single_nametags/)を格納します。
...
![名札のプレビュー画像](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/3748073/61d39c9a-0569-4abc-b6bc-06bf8d381b1f.png)
...
![participants.csvの内容](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/3748073/65ea0ac8-8822-483a-94ec-c183fa16616e.png)


これを用いて1000人分の名札を生成してみます。

## 1つのPDFファイルに全員分の名札をまとめて出力する
まずは、1つのPDFファイルに全員分の名札をまとめて出力する方法について説明します。この場合、Typstの標準機能のみで完結します。
先ほど作成したテンプレート`nametag_template.typ`を読み込み、CSVファイルから参加者情報を取得して、1人分ずつ名札を生成するTypstファイル`all_nametags.typ`を作成します。以下にその全文を示します。
...
![all_nametags.pdfのプレビュー画像](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/3748073/2926542e-4eb0-403f-8ce7-5d69a8889c96.png)
...
## 参加者ごとにPDFファイルを分割して出力する
最後に、1つのPDFファイルにまとめた名札を参加者ごとに分割して出力する方法について説明します。これにはPythonの`pypdf`ライブラリを使用するため、以下のようにインストールして下さい。

```
pip install pypdf
```

次に、以下のPythonスクリプト`split_pdf.py`を作成します。
...
if __name__ == "__main__":
    main()
```
このスクリプトは、`output/all_nametags.pdf`を読み込み、`participants.csv`から参加者名を取得して、各ページを参加者ごとに分割して`output/single_nametags/`ディレクトリに保存します。ファイル名は連番と参加者名を組み合わせた形式にしています。今回の場合、例えば`001_佐藤_太郎.pdf`のようになります。

こちらも出力にかかる時間を計測しつつ実行してみましょう。

```powershell
Measure-Command { python split_pdf.py }
```
...
Milliseconds      : 600
Ticks             : 36008636
TotalDays         : 4.1676662037037E-05
TotalHours        : 0.00100023988888889
TotalMinutes      : 0.0600143933333333
TotalSeconds      : 3.6008636
TotalMilliseconds : 3600.8636
```
すなわち、1000人分の名札を参加者ごとに分割して保存するのに約3.6秒かかりました。Typstでのコンパイル時間と比べるとやや時間がかかっていますが、これも十分に実用的な範囲内だと思います。

念のため、適当に選んだファイルの名前とPDFの中身が一致しているか確認してみます。

![split_pdf_preview.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/3748073/2cef6945-dc42-4065-9156-ba905d5b1bb7.png)

一致していることが確認できました。

## おわりに
以上、Typstを用いて紙の名札(PDF)を量産する方法について説明しました。Typstの外部データ読み込み機能を活用することで、CSVファイルから参加者情報を簡単に取り込み、効率的に名札を生成できることがわかりました。今回はテキストデータしか扱いませんでしたが、適切にパスを記述すれば画像の読み込みなども非常に簡単にできるため、例えばQRコードを名札に含めたい場合などにも応用できると思います。

Typst自体の機能も非常に強力で柔軟性が高いため、今回紹介したテンプレートをベースにして、より凝ったデザインや追加情報を含む名札を作成することも可能です。ぜひ皆さんも自分のイベントに合わせた名札を作成してみてください。
